...
#include <cstring>
  ...
  #include <intrin.h>
#elif defined(X_PMBB_ARCH_AMD64) && (defined(X_PMBB_COMPILER_GCC) || defined(X_PMBB_COMPILER_CLANG))
  #include <cpuid.h>
#endif
...
std::string xProcInfo::formatSysInfo()
{
  ...
  if(!m_ProcInfoChecked) { xDetectProcInfo(); }
...
std::string xProcInfo::xFormatMemInfo()
{
  ...
  Message += "  MemoryBasePageSize = " + std::to_string(xMemory::c_DetectedPageBase ) + " bytes\n";
  Message += "  MemoryHugePageSize = " + std::to_string(xMemory::c_DetectedPageHuge ) + " bytes\n";
  return Message;
}
xProcInfo::eMFL xProcInfo::determineMicroArchFeatureLevel()
{
  if(matchesAMD64v4()) { return eMFL::AMD64v4; } //x86-64-v4 : AVX512F, AVX512BW, AVX512CD, AVX512DQ, AVX512VL
  if(matchesAMD64v3()) { return eMFL::AMD64v3; } //x86-64-v3 : AVX, AVX2, BMI1, BMI2, F16C, FMA, LZCNT, MOVBE, XSAVE
  if(matchesAMD64v2()) { return eMFL::AMD64v2; } //x86-64-v2 : CMPXCHG16B, LAHF-SAHF, POPCNT, SSE3, SSE4.1, SSE4.2, SSSE3
  if(matchesAMD64v1()) { return eMFL::AMD64v1; } //x86-64    : CMOV, CMPXCHG8B, FPU, FXSR, MMX, FXSR, SCE, SSE, SSE2
...
void xProcInfo::xDetectProcInfo()
{
  ...
  memcpy(m_ManufacturerID  , &CPUInfo[c_RegEBX], sizeof(int32_t));
  memcpy(m_ManufacturerID+4, &CPUInfo[c_RegEDX], sizeof(int32_t));
  memcpy(m_ManufacturerID+8, &CPUInfo[c_RegECX], sizeof(int32_t));
  ...
  {
    xCPUID(CPUInfo, 0x80000002); memcpy(m_BrandString   , CPUInfo, 16);
    xCPUID(CPUInfo, 0x80000003); memcpy(m_BrandString+16, CPUInfo, 16);
    xCPUID(CPUInfo, 0x80000004); memcpy(m_BrandString+32, CPUInfo, 16);
  }
