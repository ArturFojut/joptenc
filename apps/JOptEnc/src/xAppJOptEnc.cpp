...
usage::optimization ---------------------------------------------------------
 -rol  OptQuantLuma       Apply RDOQ to luma blocks   (default 1) [optional]
...
void xAppJPEG::registerCmdParams()
{
  ...
  //optimization
  m_CfgParser.addCmdParm("rol", "OptQuantLuma"     , "", "OptQuantLuma"     );
  m_CfgParser.addCmdParm("roc", "OptQuantChroma"   , "", "OptQuantChroma"   );
...
std::string xAppJPEG::formatConfiguration()
{
  ...
  //optimization
  Config += fmt::format("OptQuantLuma      = {}\n", m_OptQuantLuma     );
  Config += fmt::format("OptQuantChroma    = {}\n", m_OptQuantChroma   );
...
void xAppJPEG::createProcessors()
{
  ...
  case eImpl::Simple:
    ...
    m_EncoderSimple.init(m_PictureSize, m_ChromaFormat, m_Quality, m_RestartIntervalUsed, true, true, true);
  ...
  case eImpl::Advanded:
    ...
    if(m_Decode)
    {
      m_DecoderParallel.setVerboseLevel(m_VerboseLevel);
      m_DecoderParallel.create(m_ThreadPool);
      m_DecoderParallel.setGatherTimeStats(m_PrintDebug);
    }
    break;
...
eAppRes xAppJPEG::processAllFrames()
{
  ...
  for(int32 f = 0; f < m_NumFrames; f++)
  {
    ...
    if(m_Decode)
    {
      ...
      case eImpl::Advanded: m_DecoderParallel.init(&m_OutBuffer); m_DecoderParallel.decode(&m_OutBuffer, m_PicRec4XX); break;
      default: return eAppRes::Error; break;
      }      
    }
...
std::string xAppJPEG::formatResultsStdOut()
{
  ...
  if(m_PrintDebug)
  {
    ...
    case eImpl::Advanded: Result += m_EncoderRDOQ  .formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
    default: break;
    }
    if(m_Decode)
    {
      ...
      case eImpl::Advanded: Result += m_DecoderParallel.formatAndResetStats("  ", m_TimeStamp.getTicksPerMicroSec()) + "\n"; break;
      default: break;
      }
    }
  }
